...
private:
  int itr = 0;
  int total_courses = 0;
  const StudentGradesInfo &instance;

public:
  StudentGradesInfoPrinter(const StudentGradesInfo &instance_)
      : instance(instance_) {}

  bool HasNext() {
    total_courses = instance.GetTotalCoursesCount();
    if (itr < total_courses)
      return true;
    return false;
  }
  ...
  pair<string, double> GetNext() {
    ...
    instance.GetCourseGradeInfo(itr++, result);
    return result;
  }

  void ResetIterator() { itr = 0; }
};
